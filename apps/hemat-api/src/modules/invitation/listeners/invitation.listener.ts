...
export class InvitationListener {
  ...
  async handleInvitationCreated(event: InvitationCreatedEvent) {
    const { email, token, invitationId, assessmentName, groupName, extraData } =
      event;
    const registerUrl = `https://africa-cdc-app-web-501628761718.us-west1.run.app/register?email=${encodeURIComponent(email)}&invitationId=${invitationId}&token=${token}`;
    const data = {
      token,
      registerUrl,
