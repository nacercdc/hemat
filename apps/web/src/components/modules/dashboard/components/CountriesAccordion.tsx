...
export const CountriesAccordion = ({ countryStatuses }: CountriesAccordionProps) => {
  const accordionContent = (
    <div className="space-y-2">
      {Object.entries(countryStatuses).map(([country, status]) => (
        <div key={country} className="flex items-center justify-between py-1 gap-4 border-b border-basic-200">
          ...
          <div className="flex w-full border border-dashed px-6" style={{ borderColor: status.color }} />
          <div
            style={{ background: status.color }}
            className={`flex items-center justify-center w-9 h-6 rounded-sm font-bold text-sm ${
